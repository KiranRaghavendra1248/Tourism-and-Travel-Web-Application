...
def add_package():
    if request.method=='POST':
        ...
        try:
            cur.execute("INSERT INTO packages values(%s,%s,%s,%s,%s,%s)",(p_id,p_name,days,destination,e_id,price))
        except:
...
def remove_package():
    if request.method=='POST':
        ...
        # Create cursor object to execute commands on MYSQL Server
        cur = mysql.connection.cursor()
        cur.execute("Delete from packages where p_id=(%s)",[p_id_])
        mysql.connection.commit()
        cur.close()
        return redirect('/all_packages')
...
def packages():
    cur = mysql.connection.cursor()
    result=cur.execute("SELECT * FROM packages")
    if result>0:
...
def add_employee():
    if request.method=='POST':
        ...
        cur = mysql.connection.cursor()
        try:
            cur.execute("INSERT INTO employee values(%s,%s,%s,%s)",(e_id,e_name,dept,role))
        except :
...
def remove_employee():
    if request.method=='POST':
        ...
        # Create cursor object to execute commands on MYSQL Server
        cur = mysql.connection.cursor()
        cur.execute("Delete from employee where e_id=(%s)",[e_id_])
        mysql.connection.commit()
        cur.close()
        return redirect('/all_employees')
...
def enquiry():
    cur = mysql.connection.cursor()
    result=cur.execute("SELECT * FROM enquiry")
    if result>0:
